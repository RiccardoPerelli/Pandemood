...
namespace Enigma_Tristezza
{
    public class TimingCrackedGlass : MonoBehaviour
    {
        ...
        private IEnumerator StartCrack()
        {
            ...
            _startCrack = true;
            MyGamepad.SetVibration(MyGamepad.StartCrack, MyGamepad.StartCrack);
        ...
        private IEnumerator StartCracking()
        {
            ...
            while (_startCrack)
            {
                ...
                if (_n < crackedGlass.Length)
                {
                    ...
                    crackGlass.Play();
                    MyGamepad.SetVibration(MyGamepad.CrackingIncrease[_n], MyGamepad.CrackingIncrease[_n]);
                }
                _n++;
                if (_n>crackedGlass.Length)
                {
                    MyGamepad.SetVibration(MyGamepad.CrackingIncrease[3], MyGamepad.CrackingIncrease[3]);
                    _startCrack = false;
