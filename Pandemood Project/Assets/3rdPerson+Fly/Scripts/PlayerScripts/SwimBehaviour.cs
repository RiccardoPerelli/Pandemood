...
namespace Scripts.PlayerScripts
{
	public class SwimBehaviour : GenericBehaviour
	{
		...
		private void Update()
		{
			...
			if (!_checkWater) return;
			if ((_waterLevel + gameObject.transform.position.y) <= _waterSurfacePosition)
			{//
				inWater = true;
			}
			if ((_waterLevel + gameObject.transform.position.y) >= _waterSurfacePosition)
		...
		private Vector3 Rotating(float horizontal, float vertical)
		{
			...
			if (_mAnimator.GetFloat(_v) >0 && _waterLevel + gameObject.transform.position.y < _waterSurfacePosition-0.1f)
			{
				targetDirection.y += 0.2f;
			}
			else if (_mAnimator.GetFloat(_v) > 0 && Math.Abs(_waterLevel + gameObject.transform.position.y - (_waterSurfacePosition-0.1f)) < 0.01)
			{
				targetDirection.y = _waterSurfacePosition;
			}
			if (_mAnimator.GetFloat(_v) < 0)
			{
		...
		{
			return isUnderwater = (_charHead + gameObject.transform.position.y) <= _waterSurfacePosition;
		}
		...
		private void OnTriggerStay(Collider other)
		{
			...
			_waterSurface = other.transform;
			_waterSurfacePosition = _waterSurface.position.y;
			_checkWater = true;
