...
public class SwimBehaviour : GenericBehaviour
{
	...
	void Start()
	{
		...
		HsumV = Animator.StringToHash("H+V");
	...
	Vector3 Rotating(float horizontal, float vertical)
	{
		...
		if (Input.GetButton(jumpButton) && WaterLevel + gameObject.transform.position.y < waterSurfacePosition)
		{
			...
			behaviourManager.GetAnim.SetFloat(direction, 1f, 0.2f, Time.deltaTime);
		}
		...
		if (behaviourManager.IsMoving()||(behaviourManager.GetAnim.GetFloat(direction)!=0&&HsumV==0&&targetDirection!=Vector3.zero))
		{
		...
		if (HsumV != 0)
		{
			if(behaviourManager.GetAnim.GetFloat(direction)>=0)
			behaviourManager.GetAnim.SetFloat(direction, 1f, 0.2f, Time.deltaTime);
			if (behaviourManager.GetAnim.GetFloat(direction) < 0)
				behaviourManager.GetAnim.SetFloat(direction, -1f, 0.2f, Time.deltaTime);
		}
		...
		if (targetDirection == Vector3.zero)
		{
			...
			behaviourManager.GetAnim.SetFloat(direction, Mathf.Lerp(direction, 0, t));
			if (behaviourManager.GetAnim.GetFloat(direction) <= 0)
			{*/
			behaviourManager.GetAnim.SetFloat(direction, 0f, 0.3f, Time.deltaTime);
			/*Vector3 rotatex = gameObject.transform.rotation.eulerAngles;
			rotatex.x = 0;
			behaviourManager.GetRigidBody.MoveRotation(Quaternion.Slerp(behaviourManager.GetRigidBody.rotation, Quaternion.Euler(rotatex), behaviourManager.turnSmoothing));
		}*/
		}
